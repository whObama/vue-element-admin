<template>
  <div class="app-container">
      ...
      <el-row :gutter="20">
        <el-col :span="4">
          <div class="search-item">
            ...
            <el-input class="input-area" placeholder="请输入角色名称" v-model="search.roleName" clearable>
            </el-input>
          </div>
        </el-col>
        <!-- <el-col :span="4">
    ...
    <el-dialog width="30%" :visible.sync="dialogVisible" :title="dialogType==='edit'?'修改角色':'添加角色'">
      <el-form :model="form" :rules="rules" ref="form" label-width="80px" label-position="left" class="demo-ruleForm">
        ...
        <el-form-item label="描述" prop="description">
          <el-input v-model="form.description" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="描述" />
        </el-form-item>
      </el-form>
    ...
    <el-dialog width="30%" :visible.sync="menuVisible" title="关联菜单">
      <el-form :model="menuform" :rules="menuRules" ref="menuform" label-width="80px" label-position="left"
        class="demo-ruleForm">
        <el-form-item label="菜单权限">
          <el-tree ref="tree" :check-strictly="checkStrictly" :data="routesData" :props="defaultProps"
            node-key="resourceCode" :default-expanded-keys="expandedNodes" show-checkbox class="permission-tree" />
        </el-form-item>
      </el-form>
      <div style="text-align:right;">
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let params = {
            ...
            resourceCodes: this.unique(selectResourceCodes)
          }
          console.log(params)
