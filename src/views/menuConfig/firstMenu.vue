<template>
  <div class="app-container">
    ...
    <!-- <el-button class="top-button" type="grey" :disabled="removeDisabled" @click="removeMore">批量删除用户</el-button> -->
    ...
    >
      ...
      <!-- <el-table-column align="center" label="图标标识">
        <template slot-scope="scope">
          {{ scope.row.resourceIcon }}
        </template>
      </el-table-column> -->
    ...
    <el-dialog width="30%" :visible.sync="dialogVisible" :title="dialogType==='edit'?'修改菜单':'添加菜单'">
      <el-form :model="form" :rules="rules" ref="form" label-width="80px" label-position="left" class="demo-ruleForm">
        ...
        <!-- <el-form-item label="菜单标识" prop="menuCode">
          <el-tooltip class="item" effect="light" content="小于等于32个字符,只允许数字和字母" placement="top">
            <el-input v-model="form.menuCode" placeholder="菜单标识" :maxlength="num32" />
          </el-tooltip>
        </el-form-item> -->
        <!-- <el-form-item label="图标标识" prop="resourceIcon">
          <el-tooltip class="item" effect="light" content="小于等于32个字符,只允许数字和字母" placement="top">
            <el-input v-model="form.resourceIcon" placeholder="图标标识" :maxlength="num32" />
          </el-tooltip>
          <div class="resourceIcon"><span style="padding-right: 4px;">图标展示:</span><i class="el-icon-share"></i></div>
        </el-form-item> -->
        <!-- <el-form-item label="菜单角色" prop="roleId">
         <el-select style="width: 100%;" v-model="form.roleId" placeholder="请选择" class="input-area">
            <el-option
...
<script>
import { queryMenuList, deleteInfo, updateMenu, getRoles } from '@/api/menuConfig'
// import { getRoles } from '@/api/role'
import { name, valueRequired, indexCode, notRequiredCode, phone } from '@/utils/regExp'
import menuConfigList  from './menuConfig'
export default {
  data() {
    return {
      rolesList: [],
  ...
  methods: {
    ...
    handleEdit(scope) {
      ...
      // this.form.menuCode = scope.row.menuCode
      this.form.resourceIcon = scope.row.resourceIcon
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          menuConfigList.map(item => {
            if (item.name === this.form.resourceCode) {
              return this.form.resourceIcon = item.icon
            }
          })
